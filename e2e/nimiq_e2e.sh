...
echo 1 | RUST_LOG=info cargo run --bin new_setup --release -- --version 2 --chunk-size 12 --powers 19 --curve mnt6_753 --upload-mode direct --server-url http://localhost:8080 --output-dir ./snark-setup-coordinator/coordinator-service/.storage --unsafe-passphrase --keys-file $BASE_DIR/nimiq-verifier.keys
