...
echo 1 | RUST_LOG=info cargo run --bin new_setup --release -- --version 1 --chunk-size 12 --powers 12 --curve mnt4_753 --circuit-filename circuit_mnt4_753 --upload-mode direct --server-url http://localhost:8080 --output-dir ./snark-setup-coordinator/coordinator-service/.storage --unsafe-passphrase --keys-file $BASE_DIR/nimiq-verifier.keys
echo 1 | RUST_LOG=info cargo run --bin new_setup --release -- --version 2 --chunk-size 8 --powers 12 --curve mnt6_753 --circuit-filename circuit_mnt6_753 --upload-mode direct --server-url http://localhost:8080 --output-dir ./snark-setup-coordinator/coordinator-service/.storage --unsafe-passphrase --keys-file $BASE_DIR/nimiq-verifier.keys
