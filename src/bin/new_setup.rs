...
use snark_setup_operator::data_structs::{
    ...
    Response, Setup, SignedData, UniqueChunkId, VerifiedData,
};
...
async fn upload_chunk<E: Pairing>(
    ...
    let round = 0;
    let path = format!("{}.{}.0", round, chunk_id);
    ...
    let location = match upload_mode {
        ...
        UploadMode::Direct => {
            ...
            format!(
                ...
                opts.server_url, round, chunk_id
            )
        }
        UploadMode::Auto => {
            return Err(anyhow!(
...
async fn run<E: Pairing>(opts: &NewSetupOpts, key_pair: &[u8]) -> Result<()> {
    ...
    for chunk_index in 0..num_chunks {
        upload_chunk::<E>(opts, phase.clone(), &mut setup, chunk_index, deployer).await?;
    }
